...
function parseUidDbText(text){
    // Fast path first; fallback to Arc parser for compatibility.
    const viaJson = safeParseJson(text, null);
    if(viaJson != null) return viaJson;
    return parseUidDbByJval(text);
}
...
function getUidDbMetaText(){
    ...
    // Avoid heavy DB parsing during startup/settings init.
    if(_uidDbCache == null){
        try{
            const fi = getUidDbFi();
...
function loadUidDb(){
    ...
    if(migratedFromSettings || !loadedFromFile){
        // Legacy data may be malformed; do full normalization once.
        const clean = {};
        for(var k in db.map){
